...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['barang'])){
		...
		$jumlah_orderan = htmlentities($_POST['jumlah_orderan']);
		...
		$data[] = $tgl;
		$data[] = $id;
		$sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, jumlah_orderan=?, merk=?, 
				harga_beli=?, harga_jual=?, satuan_barang=?, stok=?, keterangan=?, tgl_update=?  WHERE id_barang=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
